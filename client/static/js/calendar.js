class Calendar {
    ...
    async showEvents(date) {
        ...
        const filePath = `/client/static/py/calendar/calendar-data/${this.schoolFolder}/${yearMonth}.json`;
    ...
    async populateSchoolDropdown() {
        try {
            const response = await fetch('/client/static/py/calendar/school-ids.json');
            if (!response.ok) {
                throw new Error('Network response was not ok ' + response.statusText);
            }
